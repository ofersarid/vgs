export { default as TwoColumnLayout } from './two-column-layout/two-column-layout';
export { default as ThreeColumnLayout } from './three-column-layout/three-column-layout';
export { default as TwoImagesLayout } from './two-images-layout/two-images-layout';
export { default as Summary } from './summary/summary';
export { default as Clinical } from './clinical/clinical';
export { default as Cover } from './cover/cover';
export { default as IndexHeader } from './index-header/index-header';
export { default as ImgTxtBtn } from './img-txt-btn/img-txt-btn';
